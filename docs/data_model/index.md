...
|**[process](process)**|`create`<br />`terminate`|`command_line`<br />`exe`<br />`fqdn`<br />`hostname`<br />`image_path`<br />`md5_hash`<br />`parent_exe`<br />`parent_image_path`<br />`pid`<br />`ppid`<br />`sha1_hash`<br />`sha256_hash`<br />`sid`<br />`signer`<br />`user`<br />`integrity_level`<br />`parent_command_line`<br />`current_working_directory`<br />`env_vars`|
