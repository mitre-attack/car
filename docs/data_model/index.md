...
|**[http](flow)**|`get`<br />`post`<br />`put`<br />`tunnel`|`hostname`<br />`http_version`<br />`response_body_bytes`<br />`response_status_code`<br />`request_body_bytes`<br />`request_body_content`<br />`request_referrer`<br />`requester_ip_address`<br />`url_full`<br />`url_domain`<br />`url_remainder`<br />`url_scheme`<br />`url_agent_full`<br />`url_agent_name`<br />`url_agent_device`<br />`url_agent_version`|
